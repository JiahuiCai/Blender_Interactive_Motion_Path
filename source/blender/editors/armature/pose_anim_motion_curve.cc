...
typedef struct _FCurveSegment {
  ...
  bool operator==(struct _FCurveSegment const &seg) const
  {
    return (ob_name == seg.ob_name) && (pchan_name == seg.pchan_name) && (fcu == seg.fcu) &&
           (fcu->bezt[keyframe_idx[0]].vec[1][0] ==
            seg.fcu->bezt[seg.keyframe_idx[0]].vec[1][0]) &&
...
// This function keeps us from drawing multiple points at the same 3D location
// Start from the center, search in both directions
// for the first repeating pair of points
static void exclude_overlapped_points(const std::vector<FramePoint> &pts, int *first, int *last)
{
  if (pts.size() == 0) {
    return;
  }
  ...
  int *out[2] = {first, last};
  int dir[2] = {-1, 1};

  for (int d = 0; d < 2; d++) {
    FramePoint last_pt = pts[mid];
    int step = 1;
    int i = mid + (dir[d]) * step;
    bool is_overlapped = false;
    while (i >= 0 && i < pts.size() && is_overlapped == false) {
      FramePoint pt = pts[i];

      float cur_pos[3];
      float las_pos[3];

      pt.get_latest_pos(cur_pos);
      last_pt.get_latest_pos(las_pos);

      is_overlapped = equals_v3v3(cur_pos, las_pos);
  ...
  G.curves.clear();
  {
    ...
    LISTBASE_FOREACH (CollectionPointerLink *, link, &selected_pose_bones) {
      ...
      for (int i_curve = 0; i_curve < num_curves; i_curve++) {
        ...
        // exclude_overlapped_points(pts, &first, &last);
...
static void get_fcurve_segment_ex(
...
static void get_sorted_fcurve_segment(bContext *C, std::vector<FCurveSegment> &segs, FramePoint pt)
{
  Scene *scene = CTX_data_scene(C);
